    ...
    public class SQLiteHelper
    {
        ...
        public void CreateTable(SQLiteTable table)
        {
            ...
            foreach (SQLiteColumn col in table.Columns)
            {
                ...
                sb.Append(col.ColDataType.ToString().ToUpper());
        ...
        public void CopyAllData(string tableFrom, string tableTo, Dictionary<string, string> sqlColumnConversions = null)
        {
            ...
            StringBuilder sbSqlNewColumns = new StringBuilder();
            StringBuilder sbSqlConversions = new StringBuilder();

            if (sqlColumnConversions != null)
            {
                foreach (var i in sqlColumnConversions)
                {
                    if (sb.Length > 0)
                    {
                        sbSqlNewColumns.Append(",");
                        sbSqlConversions.Append(",");
                    }

                    sbSqlNewColumns.Append("`");
                    sbSqlNewColumns.Append(i.Key);
                    sbSqlNewColumns.Append("`");

                    sbSqlConversions.Append(i.Value);
                }
            }
            ...
            sb2.Append(tableTo);
            sb2.Append("`(");
            sb2.Append(sb.ToString() + sbSqlNewColumns.ToString());
            sb2.Append(") select ");
            sb2.Append(sb.ToString() + sbSqlConversions.ToString());
            sb2.Append(" from `");
        ...
        public void CopyAllData(string tableFrom, string tableTo, Action<DataRow, DataRow> conversionFunction)
        {
            DataTable dt1 = Select(string.Format("select * from `{0}` where 1 = 2;", tableFrom));
            DataTable dt2 = Select(string.Format("select * from `{0}` where 1 = 2;", tableTo));
            ...
            foreach (DataColumn dc in dt1.Columns)
            {
                if (!resultingTable.Columns.Contains(dc.ColumnName))
                {
                    resultingTable.Columns.Add(new DataColumn(dc.ColumnName, dc.DataType));
                }
            ...
            var fromDataTable = Select($"select * from '{tableFrom}'");

            foreach (DataRow row in fromDataTable.Rows)
            {
                var newRow = resultingTable.NewRow();

                foreach (DataColumn col in fromDataTable.Columns)
                {
                    if (resultingTable.Columns.Contains(col.ColumnName))
                    {
                        newRow[col.ColumnName] = row[col.ColumnName];
                    }
                }

                conversionFunction?.Invoke(row, newRow);

                resultingTable.Rows.Add(newRow);
            }

            foreach (DataRow row in resultingTable.Rows)
            {
                var insertDictionary = new Dictionary<string, object>();

                foreach (DataColumn col in resultingTable.Columns)
                {
                    insertDictionary.Add(col.ColumnName, row[col.ColumnName]);
                }

                Insert(tableTo, insertDictionary);
            }
        }
        ...
        public SQLiteTable GetTableStructure(string tableName)
        {
            var table = new SQLiteTable(tableName);
            var dt = GetColumnStatus(tableName);

            foreach (DataRow row in dt.Rows)
            {
                var name = row["name"].ToString();
                var type = (ColType)Enum.Parse(typeof(ColType), row["type"].ToString(), true);
                var notNull = Convert.ToBoolean(row["notnull"]);
                var defaultValue = row["dflt_value"].ToString();
                var isPk = Convert.ToBoolean(row["pk"]);

                table.Columns.Add(new SQLiteColumn(name, type, isPk, false, notNull, defaultValue));
            }
            return table;
        }
